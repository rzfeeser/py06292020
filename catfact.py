...
#        print(catfact.get("text"))  # the .get() method returns NONE if key not found
    ...
    # select a single dictionary from a list of dictionaries
    randomfact = random.choice(listofcatfacats["all"])

    # print the value assoc with they key text from a single dictionary
    print(randomfact["text"])

    # this line does the same thing as the top 3 lines
    # print(random.choice(r.json()["all"])["text"])

    # show the status code that the message was returned on (always a 200 if it worked)
    print(r.status_code)
